...
use App\Notifications\Sms\ExamRejectedSMS;
...
class ExamJob implements ShouldQueue
{
     ...
     * @return void
     */
    public $transcript , $answers , $transcriptRepository , $afterExamPaidSmsEnabled;
    ...
    public function handle()
    {
        ...
        try {
            ...
            if ($user_score >= $min_score) {
                ...
                $this->afterExamPaidSmsEnabled = $this->settingRepository->getRow('afterExamRejectedSmsEnabled');
                if ($this->afterExamPaidSmsEnabled == '1'){
                    Notification::send(auth()->user(), new ExamRejectedSMS($this->transcript->course_data['title']));
                }
