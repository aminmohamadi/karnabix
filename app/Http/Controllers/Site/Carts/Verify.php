...
class Verify extends BaseComponent
{
    ...
    private function success($payment = null)
    {
        ...
        if ($this->order->status == "wc-completed"){
            if (auth()->user()->referrer) {
                ...
                $user = auth()->user()->referrer;
                $user->deposit($commission, ['description' => 'order: ' . $this->order->tracking_code . '', 'from_admin'=> false]);
            }
            $amount = $this->order->price - $this->order->reductions_value - $this->order->wallet_pay;
            if ($amount > 0){
                $body = [
                ...
                $response = Http::withHeaders([
                    'Webhook-Secret' => '9B7E55EB4F1F7BAD12A29417ED52D'
                ])->withBody(json_encode($body), 'application/json')
                    ->post('https://site-payment.liara.run/webhook');
                Log::info('[POST-API-RESPONSE]', [
                    'url'    => 'https://site-payment.liara.run/webhook',
    ...
    {
        return view('new.cart.verify')->extends('new.layouts.app');
    }
}
