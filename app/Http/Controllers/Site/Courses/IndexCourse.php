...
class IndexCourse extends BaseComponent
{
    ...
    public function render(CourseRepositoryInterface $courseRepository)
    {
        ...
        $courses = $courseRepository->getAllSite($this->q ,$this->orderBy ,$this->type ,$this->category, $this->teacher,$this->property,$this->perPage);
        $coursesCount = $courseRepository->getAllSiteCount($this->q ,$this->orderBy ,$this->type ,$this->category, $this->teacher,$this->property,$this->perPage);

        $hasMore = $coursesCount > $courses->count();

        return view('new.courses.index',['courses' => $courses,'hasMore' => $hasMore])->extends('new.layouts.app');
    }
