...
class SingleCourse extends BaseComponent
{
    ...
    public function mount($slug)
    {
        ...
        if (Auth::check()){
            ...
            $this->afl = env('APP_URL')."/auth?ref=".$this->user->referral_code;
        }
        $this->page_address = [
        ...
        if ($this->actionComment != null){
            $this->replyTo = Comment::find($this->actionComment);
        }
    ...
    public function replyToComment($id)
    {
        ...
        $this->replyTo = Comment::find($id)->user->details->fullname ?? "کاربر سایت";
    ...
    public function render()
    {
        ...
        return view('new.courses.single')->extends('new.layouts.app');
    }
    ...
    public function set_content($type,$id,$title)
    {
    ...
    public function create_afl_code()
    {
        if (Auth::user()){
            $this->course->affiliates()->attach($this->user->id, [
                'code' => Str::random(8),
            ]);
            $this->afl = $this->course->affiliates()->where('user_id', $this->user->id)->first()?->pivot->code;
        }else return $this->redirect('/auth');
