...
class StoreReduction extends BaseComponent
{
    ...
    public function saveInDatabase($voucher)
    {
        ...
        $this->starts_at = $this->dateConverters($this->starts_at,'m') ;
        $this->expires_at = $this->dateConverters($this->expires_at,'m') ;
    ...
    public function dateConverters($date,$mode = 'j')
    {

        if (!empty($date)) {
            return $mode == 'j' ? Jalalian::fromDateTime(Carbon::make($date))->format('Y-m-d')
                : Jalalian::fromFormat('Y-m-d H:i:s', $this->convert2english($date))->toCarbon()->format('Y-m-d H:i:s');
        }
        return null;
    }
}
