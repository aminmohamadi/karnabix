<main class="flex-auto py-5">
                        ...
                        <div
                            ...
                            <div class="flex items-center gap-5">
                                ...
                                <div class="flex flex-col space-y-2">
                                    ...
                                    <span class="font-semibold text-xs text-muted">{{$cartContent->count()}} دوره به سبد اضافه کرده
                                            اید</span>
                                </div>
                            </div>
                        </div>
                            ...
                            @foreach($cartContent as $item)
                                ...
                                <div class="flex sm:flex-nowrap flex-wrap items-start gap-8 relative py-6">
                                    <div class="sm:w-4/12 w-full relative z-10">
                                        <div class="block">
                                            <img src="{{$item->image}}" class="max-w-full rounded-3xl"
                                                 alt="{{$item->title}}"/>
                                        </div>
                                        <button type="button"
                                                ...
                                                x-on:click="selectedItemId = {{ $item->id }}; modalOpen = true">
                                                ...
                                                <div class="flex items-center justify-between gap-5">
                                                    ...
                                                    <div class="flex flex-col items-end justify-center h-14">
                                                        @if($item->basePrice > $item->price)
                                                            <span
                                                                class="line-through text-muted">{{number_format($item->basePrice)}}</span>
                                                        @endif
                                ...
                                <div class="space-y-5">
                                    <form wire:submit.prevent="checkVoucherCode">
                                        <div class="flex items-center gap-3 relative">
                                            ...
                                            <button type="submit"
                                                ...
                                                <svg wire:target="checkVoucherCode" wire:loading.class="animate-spin" xmlns="http://www.w3.org/2000/svg" fill="none"
                                    ...
                                    <div class="flex flex-col space-y-2">
                                        <div class="flex items-center justify-between gap-3">
                                            ...
                                            <div class="flex items-center gap-1">
                                                <span
                                                    class="font-black text-base text-foreground">{{number_format(App\Http\Controllers\Cart\Facades\Cart::total())}}</span>
                                                <span class="text-xs text-muted">تومان</span>
                                            </div>
                                        </div>
                                        <div class="flex items-center justify-between gap-3">
                                            ...
                                            <div class="flex items-center gap-1">
                                                <span
                                                    class="font-black text-base text-foreground">{{ number_format($userWallet) }}</span>
                                                <span class="text-xs text-muted">تومان</span>
                                            </div>
                                        </div>
                                        <div class="flex items-center justify-between gap-3">
                                            <div class="font-bold text-xs text-foreground">مجموع استفاده از کیف پول
                                            </div>
                                            <div class="flex items-center gap-1">
                                                <span
                                                    class="font-black text-base text-foreground">{{ number_format($walletAmountShow) }}</span>
                                                <span class="text-xs text-muted">تومان</span>
                                            </div>
                                        </div>
                                        <div class="flex items-center justify-between gap-3">
                                            <div class="font-bold text-xs text-foreground"> کد تخفیف</div>
                                            <div class="flex items-center gap-1">
                                                <span
                                                    class="font-black text-base text-foreground">{{ number_format($voucherAmountShow) }}</span>
                                                <span class="text-xs text-muted">تومان</span>
                                            </div>
                                        </div>
                                        <label
                                            ...
                                            <input type="checkbox" wire:model="useWallet" class="sr-only peer"/>
                                            <div
                                    ...
                                    <div class="flex items-center justify-between gap-3 text-primary">
                                        ...
                                        <div class="flex items-center gap-1">
                                            <span
                                                class="font-black text-xl text-foreground">{{number_format(App\Http\Controllers\Cart\Facades\Cart::total($walletAmountShow,$voucherAmountShow,0))}}</span>
                                            <span class="text-xs text-muted">تومان</span>
                                        </div>
                                    </div>
                                </div>
    ...
    <span wire:loading.remove wire:target="payment" class="font-semibold text-sm flex items-center gap-1">
        تکمیل فرایند خرید
        <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20" fill="currentColor"
             class="w-5 h-5">
            <path fill-rule="evenodd"
                  ...
                  clip-rule="evenodd"></path>
                                ...
                                <span wire:loading wire:loading.class="flex"  class="font-semibold text-sm  items-center gap-1">در حال پردازش</span>

                                <svg wire:loading xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none" stroke="white" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="w-5 h-5 animate-spin">
                    ...
                    <div x-show="modalOpen" x-transition:enter="transition ease-out duration-300 transform"
                        ...
                        <div class="flex items-center gap-x-4 border-t border-border p-4">
                            ...
                            <button type="button"
                                    ...
                                    x-on:click="$wire.deleteItem(selectedItemId); modalOpen = false">
        ...
        <div>
            <div class="max-w-7xl space-y-14 px-4 mx-auto">
                <div class="flex flex-col items-center justify-center space-y-12">
                    <img src="./assets/images/theme/empty.svg" class="w-full max-w-xs opacity-35" alt="..."/>
