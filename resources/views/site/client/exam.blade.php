        ...
        <div class="dashboard-cards">
            <section class="breadcrumb-area">
                ...
                <div class="bg-primary">
                    <div class="container">
                        <div class="breadcrumb-content pt-10px pb-10px">
                            <div class="section-heading">
                                <h2 class="section__title text-white fs-30 pb-2">سوال {{ $page }} از {{ $question_count }}</h2>
                                @foreach($question as $item)
                                    <small class="text-white">سطح سوال : {{ $item->difficulty_label }}</small>
                                     <div class="section__desc text-white">
                                        {!! $item->text !!}
                                    </div>
                                @endforeach
                            </div>
            ...
            <section class="quiz-ans-wrap pt-10px pb-10px">
                <div class="container">
                                    ...
                                    <div class="row m-0 px-1">
                                        @foreach($item->choices as $choice)
                                            <div class="custom-control custom-checkbox mb-1 p-1 {{ $transcript->quiz->show_choices_type == App\Enums\QuizEnum::SHOW_SIDE_BY_SIDE ? 'col-6 col-sm-3' : 'col-12' }}  ">
                                                <input  wire:model="answers.{{$item->id}}" type="radio" id="{{ $choice->id }}" name="{{ $item->id }}" value="{{ $choice->id }}" class="" />
                                                <label class="" for="{{ $choice->id }}">
                                                    {{ $choice->title }}
                                                </label>
                                            </div>
                                        @endforeach
...
{{--                                </div>--}}
                        ...
                        <div>
                            {{$question->links('site.includes.ExamPaginate')}}
                        </div>
