{{--@php--}}
{{--    auth()->user()->wallet->balance = auth()->user()->wallet->balance + 1000;--}}
            ...
            <div class="row">
                <div class="col-lg-8 pb-5">
                                ...
                                @if(!is_null($api_bucket))
                                    ...
                                    <div class="mt-2">
                                        <button class="btn btn-outline-primary"
                                                onclick="back_to_episode('heading{{$episode_id}}')">بازگشت به درس
                                        </button>
                                    </div>
                    ...
                    <div class="course-details-content-wrap mt-5">
                                            ...
                                            <div class="card" wire:ignore>
                                                <div class="card-header" id="heading{{$item['id']}}">
                                                    <button
                                                        ...
                                                        <p>
                                                            <small
                                                                class="episode_counter">{{ $loop->iteration }}</small>
                                                            {{ $item['title'] }}
                                                        </p>

                                                        @if($item['free'] || $course->price == 0)
                                                            <small class="text-success">رایگان <i
                                                                    class="la la-lock-open">
                                                                </i></small>
                                                        @elseif(auth()->check() && $user->hasCourse($course->id))
                                                            <small class="text-success">خریداری شده <i
                                                                    class="la la-lock-open">
                                                                </i></small>
                                                ...
                                                <div id="collapse{{$item['id']}}"
                                                    ...
                                                    @if(auth()->check())
                                                        ...
                                                        @endif
                                                    @else
                                                        <p class="alert alert-info">دسترسی به این بخش نیاز به ثبت نام
                                                            دارد.</p>
                                                    @endif
                                                </div>
                                                <!-- end collapse -->
                        ...
                        {{--                                                $course->teacher->user->students_count }} دانش آموز--}}
                        {{--                                                </li>--}}
                        {{--                                                <li><i class="la la-comment-o mr-2 text-color-3"></i> {{--}}
                        {{--                                                number_format($course->teacher->user->comments_count) }} نظر--}}
                        {{--                                                </li>--}}
                        {{--                                                <li><i class="la la-play-circle-o mr-2 text-color-3"></i> {{--}}
                        {{--                                                $course->teacher->user->course_count }} دوره--}}
                        {{--                                                </li>--}}
                        {{--                                                <li><a--}}
                                        ...
                                        @isset($comments[$i])
                                                ...
                                                $course->teacher->id ? " (مدرس) " : '' }}</h5>
                                                    ...
                                                    <div class="helpful-action">
                                                        <button wire:click="$set('actionComment',{{$comments[$i]->id}})"
                                                                class="btn btn-outline-success goToCommentForm">پاسخ
                                                        </button>
                                                    </div>
                                                </div>
                                            </div>
                                            @foreach($comments[$i]->childrenRecursive as $value)
                                                <div
                                                    ...
                                                    <div class="media-img mr-4 rounded-full">
                                                        <img class="rounded-full lazy"
                                                             src="{{ asset($value->user->image) }}"
                                                             data-src="{{ asset($value->user->image) }}"
                                                             alt="{{ $value->user->name }}"/>
                                                    </div>
                                                    <div class="media-body">
                                                        <div
                                            ...
                                            (مدرس) " : '' }}</h5>
                                                        ...
                                                        <span
                                                            class="d-block lh-18 py-2">{{ $value->created_at->diffForHumans() }}</span>
                                                        <p class="pb-2">
                                                            {!! $value->content !!}
                                                        </p>
                                                    </div>
                            ...
                            <div class="see-more-review-btn text-center">
                                @if($commentCount < count($comments))
                                    <button type="button" wire:click="moreComment()"
                                            class="btn theme-btn theme-btn-transparent">بارگیری نظرات بیشتر
                                    </button>
                                @endif
                            </div>
                        </div>
                                ...
                                <div class="preview-course-feature-content pt-1 mb-">
                                    <div class="">
                                        @if($course->has_reduction && $course->base_price > 0)
                                            ...
                                            <p class="price-discount p-1">{{ $course->reduction_percent }} درصد
                                                تخفیف</p>
                                    ...
                                    <div class="buy-course-btn-box mt-4">
                                        @if ($course->price == 0)
                                            @if (auth()->check() && $user->hasCourse($course->id))
                                                <button disabled type="button"
                                                        class="btn btn-outline-success w-100 mb-2">شما در این دوره ثبت
                                                    نام کرده اید
                                                </button>
                                            @else
                                                <button wire:click="getFreeOrder()" type="button"
                                        ...
                                        @else
                                            <button wire:click="addToCart()" type="button"
                                                    class="btn theme-btn w-100 mb-2"><i
                                                    class="la la-shopping-cart fs-18 mr-1"></i> به سبد خرید اضافه کنید
                                            </button>
                                        @endif
                                    ...
                                    <div class="preview-course-incentives">
                                        ...
                                        <ul class="generic-list-item pb-3">
                                            <li><i class="la la-play-circle-o mr-2 text-color"></i>{{ $course->hours }}
                                                ساعت
                                                ویدیو اموزشی
                                            </li>
                                            ...
                                            <li>
                                                @if($afl != "")
                                                    <button class=" btn btn-primary w-100 mb-2"
                                                            onclick="copyLink('{{$afl}}')">دریافت کد
                                                        همکاری در فروش
                                                    </button>
                                                @else
                                                    <button class=" btn btn-outline-primary w-100 mb-2"
                                                            wire:click="create_afl_code()">
                                                        ایجاد کد همکاری در فروش
                                                    </button>
                                                @endif
                                            </li>
                                        </ul>
                            ...
                            <div class="card-body">
                                ...
                                <ul class="generic-list-item generic-list-item-flash">
                                    <li class="d-flex align-items-center justify-content-between">
                                        <span><i
                                                class="la la-clock mr-2 text-color"></i>مدت زمان</span> {{ $course->time }}
                                    </li>
                                    <li class="d-flex align-items-center justify-content-between">
                                    ...
                                    !is_null($course->quiz) ? 'دارد' : 'ندارد' }}
                                    </li>
                                    <li class="d-flex align-items-center justify-content-between">
                                    ...
                                    (!is_null($course->quiz) && !is_null($course->quiz->certificate)) ? 'دارد' : 'ندارد'
                                    }}
                                    </li>
                                </ul>
                            </div>
                            ...
                            <div class="card card-item">
                                <div class="card-body">
                                    ...
                                    <div class="view-all-course-btn-box">
                                        <a href="{{ route('samples') }}" class="btn theme-btn w-100">مشاهده همه نمونه
                                            سوالات <i
                            ...
                            <div class="card-body">
                                ...
                                @foreach($related_courses as $item)
                                    <div class="media media-card border-bottom border-bottom-gray pb-4 mb-4">
                                        ...
                                        <div class="media-body">
                                            <h5 class="fs-15"><a
                                                    href="{{ route('course',$item->slug) }}">{{ $item->title }}</a>
                                            </h5>
                                            <span
                                                class="d-block lh-18 py-1 fs-14">{{ $item->teacher->name ?? '' }}</span>
                    ...
                    <div>
                        @if(!is_null($homework_show) && empty($homework_show->result))
                            <button class="btn btn-sm btn-outline-danger" onclick="delete_homework()"><i
                                    class="la la-trash"></i> حذف این تمرین
                            </button>
                        @endif
                    </div>
                </div>
                        ...
                        <form wire:submit.prevent="submit_homework()">
                            <div class="row">
                                        ...
                                        <div class="form-group">
                                            <div x-data="{ isUploading: false, progress: 0 }"
                                                ...
                                                <input {{ !is_null($homework_show) ? 'disabled' : '' }} type="file"
                                                       class="custom-file-input" wire:model="homework_file"
                                                       id="homework_file">
                                                ...
                                                @if(!is_null($homework_show) && !is_null($homework_show->file))
                                                    <small class="alert d-block p-1 alert-success">فایل ارسال شده
                                                        است</small>
                                                @endif
                                                @error('homework_file')
                                ...
                                <textarea {{ !is_null($homework_show) ? 'disabled' : '' }}
                                          wire:model.defer="homework_description"
        ...
        <div class="modal-dialog modal-dialog-centered" role="document">
            <div class="modal-content">
                <div class="modal-header border-bottom-gray">
                    <h5 class="modal-title fs-19 font-weight-semi-bold" id="shareModalTitle">این دوره را به اشتراک
                        بگذارید
                    </h5>
                    <button type="button" class="close" data-dismiss="modal" aria-label="نزدیک">
                        <span aria-hidden="true" class="la la-times"></span>
...
@push('scripts')
    ...
    <script>
        ...
        function homeworkReCaptchaCallback(response) {
            @this.
            set('homework_recaptcha', response);
        }
