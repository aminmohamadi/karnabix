    ...
    @if(!$ok)
        <div class="auth-form shadow-xl rounded  mt-5 bg-white p-5">
            ...
            <form wire:submit.prevent="signUp()" id="login-with-code-form">
                ...
                <div class="form-group">
                    <label for="referral" class="label-text">کد معرف</label>
                    <input id="referral" wire:model.defer="referral" class="form-control form--control" type="text" name="referral"
                           placeholder="کد معرف">
                    @error('referral')<span class="invalid-feedback d-block">{{ $message }}</span>@enderror
